...
function makeMarker(r,i){
  ...
  const popup='<div class="popup"><div class="pt" style="color:'+c+'">'+(r.concepto||'')+' · '+(r.region||'')+' / '+(r.zona||'')+'</div><div class="pn">'+(r.nombre||'Sin nombre')+'</div>'+(r.tienda?'<div style="font-size:10px;color:#bbb;margin:-3px 0 6px;font-weight:600;letter-spacing:.3px">'+r.tienda+'</div>':'')+'<div class="pl">📍 '+(r.ciudad||'')+', '+(r.departamento||'')+'</div><div class="pcb" style="background:'+critBg+';border-left:4px solid '+c+'"><div class="pc-num" style="color:'+c+'">'+critDisp.toFixed(0)+'%</div><div class="pc-r"><div class="pniv" style="color:'+c+'">'+r.nivel+'</div><div class="pbw"><div class="pbf" style="width:'+Math.min(100,critDisp)+'%;background:'+c+'"></div></div>'+avgLine+dHTML+'</div></div><div class="psc">'+scoreRows+'</div><div class="pkp"><div class="pk"><div class="pkv">'+Math.round(r.retiros||0)+'</div><div class="pkl">Retiros</div></div><div class="pk"><div class="pkv">'+Math.round(r.dias_aus||0)+'d</div><div class="pkl">Ausentismo</div></div><div class="pk"><div class="pkv">'+Math.round(r.accidentes||0)+'</div><div class="pkl">Accidentes</div></div><div class="pk"><div class="pkv">'+r.horas_extra_ratio.toFixed(1)+'%</div><div class="pkl">H.Extra</div></div><div class="pk"><div class="pkv">'+Math.round(r.proc_disc||0)+'</div><div class="pkl">Proc.Disc.</div></div><div class="pk"><div class="pkv">'+Math.round(r.quejas||0)+'</div><div class="pkl">Quejas</div></div><div class="pk"><div class="pkv">'+((r.productividad_hc||0).toFixed(1))+'</div><div class="pkl">Product./HC</div></div></div><div class="ppl"><strong>AM:</strong> '+(r.am||'N/A')+'<br><strong>DM:</strong> '+(r.dm||'N/A')+'</div></div>';
...
function renderKPIs(data){
  ...
  top10el.innerHTML=[...data].sort((a,b)=>b.crit_pct-a.crit_pct).slice(0,10).map((r,i)=>{const c=COLOR[r.nivel]||'#888';const hD=r.delta!==null;const dUp=hD&&r.delta>0;const dStr=!hD?'':dUp?'▲ +'+r.delta.toFixed(1)+'%':r.delta<0?'▼ '+r.delta.toFixed(1)+'%':'=';const dCl=!hD?'#ccc':dUp?'#CC0000':'#27A243';return'<div class="ti" style="cursor:pointer" onclick="focusItem(currentData.findIndex(x=>x.ceco===\''+r.ceco+'\'))"><span class="tr">'+(i+1)+'</span><div class="tn"><div class="tnm">'+(r.nombre||'Sin nombre')+'</div><div class="tc">'+(r.ciudad||'')+' · '+(r.region||'')+'</div></div><div class="tt"><div class="tp" style="color:'+c+'">'+r.crit_pct.toFixed(0)+'%</div>'+(hD?'<div class="td2" style="color:'+dCl+'">'+dStr+'</div>':'')+'</div></div>';}).join('');
}
