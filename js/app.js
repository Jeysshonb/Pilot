...
function renderKPIs(data){
  ...
  top10el.innerHTML=[...data].sort((a,b)=>b.crit_pct-a.crit_pct).slice(0,10).map((r,i)=>{const c=COLOR[r.nivel]||'#888';const hD=r.delta!==null;const dUp=hD&&r.delta>0;const dStr=!hD?'':dUp?'▲ +'+r.delta.toFixed(1)+'%':r.delta<0?'▼ '+r.delta.toFixed(1)+'%':'=';const dCl=!hD?'#ccc':dUp?'#CC0000':'#27A243';return'<div class="ti" style="cursor:pointer" onclick="showTab(\'lista\');focusItem(currentData.findIndex(x=>x.ceco===\''+r.ceco+'\'))"><span class="tr">'+(i+1)+'</span><div class="tn"><div class="tnm">'+(r.nombre||'Sin nombre')+'</div><div class="tc">'+(r.ciudad||'')+' · '+(r.region||'')+'</div></div><div class="tt"><div class="tp" style="color:'+c+'">'+r.crit_pct.toFixed(0)+'%</div>'+(hD?'<div class="td2" style="color:'+dCl+'">'+dStr+'</div>':'')+'</div></div>';}).join('');
}
